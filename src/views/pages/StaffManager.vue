    ...
    <div>
        <el-row type='flex'>
            <el-card style='min-height:600px;width:250px;min-width:220px;'>
                <scroll v-loading="scrollLoading" @ScrollItemClick='ScrollItemClick' :list='department' title='部门选择' />
            </el-card>
            <el-card style='width:100vw;overflow-x:hidden;margin-left:8px'>
                <el-form>
                    <el-form-item label='搜索：'>
                        <el-autocomplete :trigger-on-focus='false' prefix-icon="el-icon-search" size='mini' v-model="searchValue" :fetch-suggestions="querySearchAsync" placeholder="输入工号或者姓名搜索"
                    @select="handleSelect"></el-autocomplete>
                    </el-form-item>
                </el-form>
                <el-form inline>
                    <el-form-item label='工号：'>
                        <el-input size='mini' prefix-icon='el-icon-data-analysis' type="text"
                            v-model="queryParams.empNo" autocomplete="off" placeholder="工号"></el-input>
                    </el-form-item>
                    <el-form-item label='姓名：'>
                        <el-input size='mini' style='width:100px' prefix-icon='el-icon-data-analysis' type="text"
                            v-model="queryParams.empName" autocomplete="off" placeholder="姓名"></el-input>
                    </el-form-item>
                    <el-form-item label='职级：'>
                        <el-select size='mini' v-model="queryParams.rankId" placeholder="选择职级">
                            <el-option v-for="(item,key) in  selectOption" :key="key" :label="item.rankName"
                                :value="item.rankId"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label=''>
                ...
                <el-button @click='addEmp(null,1)' v-if='queryParams.deptCode!==null' size='small' type="primary">添加部长
                </el-button>
                <el-table :expand-row-keys='expand' :row-key='getRowkey' @expand-change='expandChange'
                    v-loading="tableLoading" :data="dataSource">
                    <el-table-column type="expand">
                        <template slot-scope="scope">
                            <el-form>
                                <el-form-item label="操作：">
                                    <el-button @click='addEmp(scope.row,2)' v-if='scope.row.rank.rankId!==1'
                                        size='small' type="primary">添加</el-button>
                                    ...
                                    <el-button @click='overTimeDetail(scope.row)' size='small' type="warning">查看加班情况
                                    </el-button>
                                </el-form-item>
                            </el-form>
                        </template>
                ...
                <el-row type="flex" justify="center">
                    <el-pagination background :total="paginationConfig.total"
                        :current-page="paginationConfig.currentPage" :page-size="paginationConfig.pageSize"
                        @current-change="currentChange" layout="prev, pager, next,jumper, total">
                    </el-pagination>
                </el-row>
            </el-card>
        </el-row>
            ...
            <el-row type="flex" justify="center">
                <el-form>
                    <el-form-item label="部门名称：">
                        <el-input disabled prefix-icon='el-icon-data-analysis' type="text" v-model="dialogForm.deptName"
                            autocomplete="off" placeholder="部门名称"></el-input>
                    </el-form-item>
                    <el-form-item label="员工名称：">
                        <el-input prefix-icon='el-icon-data-analysis' type="text" v-model="dialogForm.empName"
                            autocomplete="off" placeholder="员工名称"></el-input>
                    </el-form-item>
                    <el-form-item label="员工工号：">
                        <el-input prefix-icon='el-icon-data-analysis' type="text" v-model="dialogForm.empNo"
                            autocomplete="off" placeholder="员工工号"></el-input>
                    </el-form-item>
                    <el-form-item label="领导名称：">
                        <el-input disabled prefix-icon='el-icon-data-analysis' type="text"
                            v-model="dialogForm.leaderName" autocomplete="off" placeholder="领导名称"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="">
                        ...
                        <el-button size='small' @click="()=>{this.dialogConfig.visible=false;this.restDialogForm()}">取消
                        </el-button>
                    </el-form-item>
                </el-form>
            </el-row>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.$store.dispatch('departmentManager/departmentListWithoutPage')
                .then(res => {
                    setTimeout(() => {
        ...
        methods: {
            ...
            submitEmp() {
                if (this.dialogConfig.mode === 3) {
                    ...
                    this.$store.dispatch('staffManager/addStaff', this.dialogForm)
                        .then(res => {
                            this.restDialogForm()
            ...
            delEmp(row) {
                console.log(row)
                this.$store.dispatch('staffManager/delStaff', row.empNo)
                    .then(res => {
                        if (res.code === '200') {
                            Message({
                                message: res.message,
            ...
            queryWithFilter(pageNum) {
                ...
                this.$store.dispatch('staffManager/getStaff', this.queryParams)
                    .then(res => {
                        setTimeout(() => {
